...
### Node.js — `child_process.spawn`, `fs`, etc.

* Documentation officielle `child_process.spawn()` et les alternatives (`exec()`, `execFile()`, etc.) ([nodejs.org][11])
* Article « Node.js Child Processes: Everything you need to know » — comparaisons entre spawn, exec, fork, etc. ([FreeCodeCamp][12])
* GeeksforGeeks — résumé sur `child_process.spawn()` et ses usages ([GeeksforGeeks][13])
* DigitalOcean — exemples concrets de l’utilisation de `spawn` dans Node.js ([DigitalOcean][14])
